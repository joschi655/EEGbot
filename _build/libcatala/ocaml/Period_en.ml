...
module Period = struct
  ...
  let rtype = Value.Struct {
    name = "Period";
    fields = fun t -> [
      "begin", Value.embed (Value.Date) t.begin__1;
      "end", Value.embed (Value.Date) t.end__1;
    ]
  }
end
...
let loc : code_location array =
  [|{filename="libcatala/period_en.catala_en"; start_line=25; start_column=7;
     end_line=25; end_column=29;
    ...
    {filename="libcatala/period_en.catala_en";
     ...
     law_headings=["Definitions and operations"; "Periods of time"]};
    {filename="libcatala/period_en.catala_en"; start_line=56; start_column=5;
     end_line=56; end_column=18;
     law_headings=["Definitions and operations"; "Periods of time"]};
    {filename="libcatala/period_en.catala_en";
     start_line=66; start_column=14; end_line=66; end_column=30;
     law_headings=["Definitions and operations"; "Periods of time"]}|]
...
let of_month_and_year : Date_en.Month.t -> integer -> Period.t =
  fun (pmonth: Date_en.Month.t) (pyear: integer) ->
    let pbegin : date =
      ((Date_en.of_year_month_day) (loc.(0)) pyear
         ((Date_en.month_to_integer) pmonth) (integer_of_int 1))
    in
...
let of_year : integer -> Period.t =
  fun (pyear: integer) ->
    {Period.begin__1 =
       ((Date_en.of_year_month_day) (loc.(1)) pyear (integer_of_int 1)
          (integer_of_int 1));
       Period.end__1 =
         ((Date_en.of_year_month_day) (loc.(2)) pyear (integer_of_int 12)
            (integer_of_int 31))}

(* Toplevel def valid *)
let valid : Period.t -> bool =
  fun (p: Period.t) -> o_not ((p.Period.end__1) < (p.Period.begin__1))

(* Toplevel def duration *)
let duration : Period.t -> duration =
  fun (p: Period.t) ->
    o_sub_dat_dat
      (o_add_dat_dur AbortOnRound (loc.(3)) (p.Period.end__1)
         (duration_of_numbers 0 0 1)) (p.Period.begin__1)

(* Toplevel def are_adjacent *)
let are_adjacent : Period.t -> Period.t -> bool =
  fun (p1: Period.t) (p2: Period.t) ->
    (p1.Period.end__1)
      = (o_sub_dat_dur AbortOnRound (loc.(4)) (p2.Period.begin__1)
  ...
  fun (p1: Period.t) (p2: Period.t) ->
    let intersection__1 : Period.t =
      {Period.begin__1 =
         ...
         Period.end__1 =
           ((Date_en.min) (p1.Period.end__1) (p2.Period.end__1))}
    in
    if valid intersection__1
    then (Optional.Present intersection__1)
    else Optional.Absent

(* Toplevel def overlaps *)
let overlaps : Period.t -> Period.t -> bool =
  fun (p1: Period.t) (p2: Period.t) ->
    match (intersection p1 p2) with
...
let covers : Period.t -> Period.t -> bool =
  fun (long: Period.t) (short: Period.t) ->
    ((long.Period.begin__1) <= (short.Period.begin__1)) &&
      ((short.Period.end__1) <= (long.Period.end__1))

(* Toplevel def is_contained *)
let is_contained : Period.t -> date -> bool =
  fun (p: Period.t) (d: date) ->
    ((p.Period.begin__1) <= d) && (d <= (p.Period.end__1))

(* Toplevel def is_before *)
let is_before : Period.t -> date -> bool =
  fun (p: Period.t) (d: date) -> d < (p.Period.begin__1)

(* Toplevel def is_after *)
let is_after : Period.t -> date -> bool =
  fun (p: Period.t) (d: date) -> d > (p.Period.end__1)
    ...
    o_map
      (fun (tpl: ((date * date) * 'tt)) ->
         ((let args : (date * date) = (let x, _ = tpl in x) in
           of_tuple (let x, _ = args in x) (let _, x = args in x)),
          (let _, x = tpl in x))) l
...
let sort_by_date : ((Period.t * 'tt) array) -> ((Period.t * 'tt) array) =
  fun (l: (Period.t * 'tt) array) ->
    let tuple_associated_list : ((date * date) * 'tt) array =
      (to_tuple_associated_list l)
    in
    let sorted_tuple_associated_list : ((date * date) * 'tt) array =
      ((Period_internal.sort_) tuple_associated_list)
    in
...
let () =
  Catala_runtime.register_module "Period_en"
    ...
    "CM0|30c623e2|32e49b03|02eca492"
